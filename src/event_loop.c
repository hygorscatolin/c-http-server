...
static void send_response_and_close(int client_fd) {
     ...
     * expected -- not worth a retry/backpressure path before layer 3
...
static void handle_client_readable(int client_fd) {
    ...
    for (;;) {
        ...
        if (n > 0) {
            /* Payload is discarded -- this layer doesn't parse requests
             * yet, it just needs to fully drain the socket per the ET
             * contract above. Note this does mean a client trickling an
             * enormous body could hog this single-threaded loop; fine for
             * a hello-world responder, not for a real server. */
            continue;
        }
        if (n == 0) {
 ...
 * complete their handshake between two epoll_wait() calls -- we'd never
